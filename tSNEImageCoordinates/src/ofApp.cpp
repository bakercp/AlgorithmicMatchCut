...
-------------------------- Not important for debugging */
    ...
    for (ofJson::iterator it = inputJson.begin(); it != inputJson.end(); ++it) {
        //cout << inputJson[it.key()].size() << endl; //makes sense, returns 43 numbers, the numbers of how many images are in each sequence
        
        for (int i = 0; i < inputJson[it.key()].size(); i++) {
            std::vector<double> v;
            imagePoints.push_back(v);
            //cout << &v << endl; //lol wot, i dont know what to look at here
